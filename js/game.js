...
let enemyPseudo = "Adversaire"; // Sera mis à jour par le réseau
...
function genererCatalogue() {
    GENRES.forEach(genre => {
        for(let e=1; e<=5; e++) catalogueComplet.push({ genre: genre, etoiles: e });
    });
}
...
function ajouterAuDeck(carte) {
    ...
    if (compteurEtoiles[carte.etoiles] >= LIMITES_ETOILES[carte.etoiles]) return alert(`Tu as atteint la limite de cartes ${carte.etoiles} étoiles !`);

    monDeckPersonnalise.push({ ...carte, id: `C${monDeckPersonnalise.length}`, type: 'combat' });
...
function actualiserUIBuilder() {
    ...
    btn.innerText = `Valider (${monDeckPersonnalise.length}/60)`;
...
function configurerConnexion() {
    ...
    networkConn.on('data', (data) => {
        if (data.type === 'hello') {
            enemyPseudo = data.pseudo;
            document.getElementById('enemy-name-ready').innerText = enemyPseudo;
            document.getElementById('enemy-name-score').innerText = enemyPseudo;
        }
            ...
            enemyTrain.addWagon(data.carteObj, true, 'combat'); 
            if (data.fast) enemyTrain.speedUp();
        }
        else if (data.type === 'special_played') {
            let targetIsJoueur = !data.isTrainJoueur;
            ajouterBadgeVisuel(targetIsJoueur, data.index, data.carteObj);
        }
    });
}
...
class TrainController {
    ...
    addWagon(carteObj, isHidden, type) { 
        let wagonDiv = document.createElement('div'); wagonDiv.className = this.isPlayer ? `wagon-ingame type-${type}` : 'enemy-wagon-ingame'; 
        let indexWagon = this.wagons.length; wagonDiv.dataset.index = indexWagon; 
        wagonDiv.onclick = () => clicSurWagon(this.isPlayer, indexWagon); 
        
        if (isHidden) { 
            wagonDiv.innerText = "?"; 
            wagonDiv.dataset.realGenre = carteObj.genre; wagonDiv.dataset.realEtoiles = carteObj.etoiles; wagonDiv.dataset.realType = type; 
        } else { 
            wagonDiv.innerHTML = `<span class="card-genre">${carteObj.genre}</span><span class="card-stars">${'⭐'.repeat(carteObj.etoiles)}</span>`;
            wagonDiv.dataset.stars = carteObj.etoiles; // Stockage pour le combat
        } 
    ...
    // Reveal complet à la fin du chrono
    revealWagons() { 
        this.wagons.forEach(w => { 
            if (w.dataset.realGenre) { 
                w.innerHTML = `<span class="card-genre">${w.dataset.realGenre}</span><span class="card-stars">${'⭐'.repeat(w.dataset.realEtoiles)}</span>`;
                w.className = `wagon-ingame type-${w.dataset.realType} reveal-anim`;
                w.dataset.stars = w.dataset.realEtoiles; // Définit la valeur de combat
            } 
            // On révèle les badges cachés !
            w.querySelectorAll('.hidden-badge').forEach(b => {
                b.classList.remove('hidden-badge');
                w.dataset.badgeMod = b.classList.contains('badge-soutien') ? 1 : -1; // Pour le calcul
            });
        }); 
    }

    clearBoard() {
        this.loco.style.display = 'none';
        this.wagons.forEach(w => w.remove());
        this.wagons = []; this.history = []; this.targetSteps = [];
    }
}
...
function demarrerRound() {
    ...
    if(playerTrain) playerTrain.clearBoard();
    if(enemyTrain) enemyTrain.clearBoard();

    playerTrain = new TrainController(true); enemyTrain = new TrainController(false);
    
    let nbCartes = 8;
    if (currentRound === 3 && myScore === 2 && enemyScore === 0) {
        nbCartes = 6; alert("Attention : Tu mènes 2-0. Main réduite à 6 cartes !");
    } else if (currentRound > 1) {
        nbCartes = 4; // Pioche classique tours suivants
    }
...
async function animerDistribution(nbCombat) {
    let cartesAPiocher = [...deckCombat.splice(0, nbCombat), ...deckSoutien.splice(0, currentRound===1?3:0), ...deckSabotage.splice(0, currentRound===1?3:0)];
...
function jouerCarteMonstre(idCarte) { 
    ...
    let isFast = tempsRestant >= (tempsTotalPhase / 2); let index = mainDuJoueur.findIndex(c => c.id === idCarte); let carte = mainDuJoueur[index]; 
    mainDuJoueur.splice(index, 1); afficherMain(); 
    playerTrain.addWagon(carte, false, carte.type); 
    if (isFast) playerTrain.speedUp(); 
    if (networkConn && networkConn.open) networkConn.send({ type: 'card_played', etape: etapeGare, carteObj: carte, fast: isFast }); 
}
...
function placerCarteSpeciale(isTrainJoueur, indexWagon, carte) { 
    ...
    if (networkConn && networkConn.open) networkConn.send({ type: 'special_played', isTrainJoueur: isTrainJoueur, index: indexWagon, carteObj: carte }); 
}

// Le badge a maintenant la classe 'hidden-badge' pour ne pas être vu avant le reveal !
function ajouterBadgeVisuel(isTrainJoueur, indexWagon, carte) { 
    let train = isTrainJoueur ? playerTrain : enemyTrain; let wagonDiv = train.wagons.find(w => w.dataset.index == indexWagon); if (!wagonDiv) return; 
    let badge = document.createElement('div'); badge.className = `badge-${carte.type} hidden-badge`; badge.innerText = carte.label; wagonDiv.appendChild(badge); 
}
...
function demarrerPhaseSoutienSabotage() {
    phaseSoutienSabotage = true; document.getElementById('phase-title').innerText = "✨ SOUTIEN & SABOTAGE ✨"; document.getElementById('instruction-text').innerText = "Place tes spéciaux !"; document.getElementById('timer-display').style.display = 'block'; afficherMain(); lancerTimer(25);
}
...
async function terminerPhaseSpeciale() {
    ...
    document.getElementById('instruction-text').innerText = "Révélation..."; document.getElementById('timer-display').style.display = 'none';
    
    // Révèle les cartes ennemies et TOUS les badges cachés !
    enemyTrain.revealWagons();
    playerTrain.revealWagons(); 

    // Attend un peu pour voir le résultat du reveal
...
async function resoudreCombats() {
    document.getElementById('instruction-text').innerText = "Bataille en cours !";
    
    let pSurivants = 0; let eSurivants = 0;

    // Le Wagon Gare 1 (le premier posé) est à l'index 4 de l'array, Gare 5 est à l'index 0.
    // On résout de Gare 1 à Gare 5 (donc de l'arrière vers l'avant sur l'écran)
    for(let i=4; i>=0; i--) {
        let pW = playerTrain.wagons[i];
        let eW = enemyTrain.wagons[i];

        // Lancement de l'animation CSS d'entrechoc
        pW.classList.add('clash-player');
        eW.classList.add('clash-enemy');
        await sleep(600); // Laisse l'animation taper

        // Calcul Mathématique : Etoiles + Modificateur de Badge
        let pStars = parseInt(pW.dataset.stars) + (parseInt(pW.dataset.badgeMod) || 0);
        let eStars = parseInt(eW.dataset.stars) + (parseInt(eW.dataset.badgeMod) || 0);

        if (pStars > eStars) {
            eW.classList.add('faint'); pSurivants++;
        } else if (eStars > pStars) {
            pW.classList.add('faint'); eSurivants++;
        } else {
            // Egalité : les deux meurent
            pW.classList.add('faint'); eW.classList.add('faint');
        }
        await sleep(500); // Pause avant le prochain duel
    }

    await sleep(1000);

    // Bilan du Round
    if (pSurivants > eSurivants) { myScore++; document.getElementById('instruction-text').innerText = "🎉 TU GAGNES CE ROUND !"; }
    else if (eSurivants > pSurivants) { enemyScore++; document.getElementById('instruction-text').innerText = "💀 ROUND PERDU..."; }
    else { document.getElementById('instruction-text').innerText = "⚖️ ÉGALITÉ SUR CE ROUND !"; }

    document.getElementById('my-score').innerText = myScore;
    document.getElementById('enemy-score').innerText = enemyScore;

    await sleep(3000); // Temps pour apprécier la victoire
    
    currentRound++;
    demarrerRound(); // On relance la boucle complète
}
